...
person<- data.frame(Risk = 0.6, NEP = 15, income= "very_rich", age = "tothirty", probresponse = 0.7)

personbid <-predict(bidlm, newdata=person, type = "response")

person2<- data.frame(Risk = 0.6, NEP = 37, income= "middle", age = "tothirty", probresponse = 0.7)
personbid2 <-predict(bidlm, newdata=person2, type = "response")

final <-predicteddata %>% 
  select(NEP, income, age) %>% 
  mutate(probresponse = 0.7) %>% 
  mutate(Risk= 0.6) %>% 
